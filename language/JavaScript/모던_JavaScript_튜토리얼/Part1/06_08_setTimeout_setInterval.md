...
### 4. 대기 시간이 0인 setTimeout

- `setTimeout(func, 0)`이나 `setTimeout(func)`을 사용하면 `setTimeout`의 대기 시간을 0으로 설정할 수 있음
- 이렇게 대기 시간을 0으로 설정하면 `func`을 '가능한 한' 빨리 실행할 수 있음
- 다만, 이 때 스케줄러는 현재 실행 중인 스크립트의 처리가 종료된 이후에 스케줄링한 함수를 실행함
- 이런 특지을 이용하면 현재 스크립트의 실행이 종료된 '직후에' 원하는 함수가 실행될 수 있게 할 수 있음
- 예시를 실행하면 얼럿창에 'Hello'와 'World'가 순서대로 출력되는 것을 확인할 수 있음
...
- 예시에서 첫 번째 줄은 '0밀리초 후에 함수 호출하기'라는 할 일을 '계획표에 기록'해주는 역할을 함
- 그런데 스케줄러는 현재 스크립트(alert함수)의 실행이 종료되고 나서야 '계획표에 어떤 할 일이 적혀있는지 확인'하므로, `Hello`가 먼저, `World`은 그 다음에 출력됨
- 대기 시간이 0인 setTimeout을 활용한 브라우저 환경에서의 유스 케이스는 이벤트 루프와 매크로태스크, 마이크로태스크에서 자세히 다룰 것

- NOTE : 브라우저 환경에서 실제 대기 시간은 0이 아님

  - 브라우저는 HTML5 표준에서 정한 중첩 타이머 실행 간격 관련 제약을 준수함
  - 해당 표준엔 "다섯 번째 중첩 타이머 이후엔 대기 시간을 최소 4밀리초 이상으로 강제해야 한다"라는 제약이 명시되어 있음
  - 예시
  ...
  - 이 제약은 `setTimeout` 뿐만 아니라 `setInterval`에도 적용됨
  - `setInterval(f)`도 처음 몇 번은 함수 f를 지연 없이 실행하지만, 나중엔 지연 간격을 4밀리초 이상으로 늘려버림
  - 이는 오래전부터 있었던 제약
  - 구식 스크립트 중 일부는 아직 이 제약에 의존하는 경우가 있어, 명세서를 변경하지 못하고 있는 상황
  - 한편, 서버 측엔 이런 제약이 없음
  - Node.js의 process.nextTick과 setImmediate를 이용하면 비동기 작업을 지연 없이 실행할 수 있음
