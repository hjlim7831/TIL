7- 자바스크립트는 함수를 다룰 때 탁월한 유연성을 제공함
...
- 함수 또는 메서드를 데코레이터로 감싸 대체하는 것은 대체적으로 안전함
- 원본 함수에 `func.calledCount` 등의 프로퍼티가 있으면 데코레이터를 적용한 함수에선 프로퍼티를 사용할 수 없으므로, 안전하지 않음
- 함수에 프로퍼티가 있는 경우엔 데코레이터 사용에 주의해야 함

- 위 예시에서 함수 slow에 프로퍼티가 있었다면 `cachingDecorator(slow)` 호출 결과인 래퍼엔 프로퍼티가 없을 것

- 몇몇 데코레이터는 자신만의 프로퍼티를 갖기도 함
- 데코레이터는 함수가 얼마나 많이 호출되었는지를 세거나, 호출 시 얼마나 많은 시간이 소모되었는지 등의 정보를 래퍼의 프로퍼티에 저장할 수 있음
- 함수 프로퍼티에 접근할 수 있게 해주는 데코레이터를 만드는 방법도 있음
- 이걸 구현하려면 `Proxy`라는 특별한 객체를 사용해 함수를 감싸야 함
- `Proxy`에 대해선 `Proxy와 Reflect`에서 다루도록 할 것
